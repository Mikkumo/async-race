...
class Car {
    ...
    constructor(id: number, name: string, color: string) {
        ...
        this.container.className = 'car_container';
        this.carId = id;
        this.name = name;
        this.color = color;
    }

    createCarContainer(context: Context, color: string) {
        const container = new Div('car_container').render();
        container.append(this.createButtons());
        container.append(this.createRoad(context, color));
        return container;
    }

    createButtons() {
        const buttons = new Div('car_buttons').render();
        const selectBtn = new Button('button select_btn', `select_car_${this.carId}`, 'select').render();
        selectBtn.addEventListener('click', () => {
            const garageController = new GarageController();
            console.log('delete');
            const form = document.getElementsByClassName('update_form')
            
            garageController.deleteGarage(garage, this.carId);
        })
        const removeBtn = new Button('button remove_btn', `remove_btn_${this.carId}`, 'remove').render();
        ...
        buttons.append(new TextComponent('span', 'car_name', `${this.name}`).render());
        return buttons;
    }

    createRoad(context: Context, color: string) {
        const road = new Div('road').render();
        const controlPanel = new Div('control_panel').render();
        const greenBtn = new Button('button start_btn', `start_btn_${this.carId}`, '🟢').render()
        const redBtn = new Button('button stop_btn disabled', `stop_btn_${this.carId}`, '🔴').render()
        const id = this.carId
        greenBtn.addEventListener('click', function() {
            ...
            const garageController = new GarageController()
            garageController.manageEngine(context, id, "started")
        });
        controlPanel.append(greenBtn);
        redBtn.addEventListener('click', function() {
            ...
            const garageController = new GarageController()
            garageController.manageEngine(context, id, "stopped")
        });
        controlPanel.append(redBtn);
        road.append(controlPanel);
        const car = new Div('car', `car_${this.carId}`).render();
        const carSvg = this.renderCarImage(color);
        car.append(carSvg);
        road.append(car);
        road.append(new Div('flag', `flag_${this.carId}`, '🏁').render());
        return road;
    }

    renderCarImage(color: string) {
        const car = document.createElementNS('http://www.w3.org/2000/svg','svg');
        car.setAttributeNS(null, 'viewBox', '0 0 512 512');
        car.setAttributeNS(null, 'class', 'car');
        car.setAttributeNS(null, 'fill', `${color}`);
    ...
    render(context: Context) {
        ...
        this.container.append(this.createRoad(context, this.color));
